...
public class Leet912 {
    ...
    static void  quickSort(int[] nums, int beginIndex, int endIndex) {
        int left = beginIndex;
        int right = endIndex;
        if(left < right) {
            int temp = nums[left];
            ...
            nums[left] = temp;
            quickSort(nums,beginIndex, left);
     ...
     * 在初始序列较大时取较大的步长，通常取序列长度的一半，此时只有两个元素比较，
     * 交换一次；之后步长依次减半直至步长为1，即为插入排序，
     ...
     * 最后，遍历桶元素，并依次将值非0的元素下标值载入排序数列（桶元素>1表明有值大小相等的元素，此时依次将他们载入排序数列），
     * 遍历完成，排序数列便为有序数列。
     *
     * 时间复杂度：O(x*N) 　　稳定性：稳定
     * @param arr
